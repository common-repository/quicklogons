...
class LandingProvider
{
    ...
    public function landOn()
    {
        ...
        if ($hash != $p->hash) return new LandingParameters(array("error" => __("Hash is incorect", "quicklogons")));

        $utc_zone = new DateTimeZone("UTC");
        $rd = DateTime::createFromFormat("Y-m-d H:i:sZ", $p->timestamp, $utc_zone);
        $cd = new DateTime("now", $utc_zone);
        if ($cd > $rd->modify("+60 second")) return new LandingParameters(array("error" => __("Request expired", "quicklogons")));

        if (isset($p->error) && $p->error > "" || isset($p->user_id) && $p->user_id > "") return $p;

        return new LandingParameters(array("error" => __("Wrong UserId", "quicklogons")));
    }
